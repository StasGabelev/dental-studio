...
6. ЗАПРЕТ НА ВЫДУМКУ ДАННЫХ: При показе статистики из инструментов — показывай ТОЛЬКО то, что реально вернул инструмент. Не добавляй врачей, пациентов, цифры которых нет в ответе инструмента. Если инструмент вернул 2 врача — показывай 2, не 6.
- Для вопросов "скільки зубів лікують по понеділках", "скільки терапевтів по понеділках", "скільки зубів в цей/минулий понеділок/тиждень/місяць/рік", "терапевт по понеділках" → вызывай get_monday_therapy_stats. Передавай параметр period: this_monday/last_monday/this_week/last_week/this_month/last_month/this_year/last_year/last_90_days. Ключові послуги терапевта: "Фотополімерна реставрація", "Лікування С молочного зуба", "Лікування С постійного зуба", "Первинне ендолікування", "Первинне ендолікування під мікроскопом".`;
