...
# Defaults mapping using pure english keys and pure unicode escapes.
# Since we write this in Python string, we can just do pure unicode.
defaults = {
...
page_start = content.find('const PAGE_SCHEMA = {')
about_start = content.find('"about": [', page_start)
home_section = content[page_start:about_start]

for key, val in defaults.items():
    escaped_val = val.replace('"', '\\"') # escape quotes
    pattern = r'("key":\s*"' + re.escape(key) + r'",\s*"label":\s*"[^"]+",\s*"type":\s*"[^"]+")'
    replacement = r'\1,\n            "default": "' + escaped_val + r'"'
    home_section = re.sub(pattern, replacement, home_section)

content = content[:page_start] + home_section + content[about_start:]
